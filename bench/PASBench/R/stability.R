...
species = "human" #args[4]
organ = "lung" #args[5]
data = args[6]
if(species == 'human'){
  gSets_path = "c:\\Users\\user\\Documents\\PASBench\\inst\\gmtFiles\\human\\kegg.gmt"
}else{
  gSets_path = './gmtFiles/mouse/kegg.gmt'
}

#data_path = file.path('./dataSets',species, organ,paste0(data,'.rds'))
data_path = file.path('c:\\Users\\user\\Documents\\PASBench\\dataSets\\human\\lung\\li.rds')

start = as.integer(args[2])
end = as.integer(args[3])
out_folder = file.path('../results/stability/')
...
print(dim(counts))
eval = cal_all_tools(counts, gSets_path, tools = c('AUCell'), filter = FALSE, normalize = 'none')
